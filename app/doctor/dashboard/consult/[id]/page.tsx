...
export default function ConsultDetailPage({ params }: { params: { id: string } }) {
      ...
      try {
        ...
        if (!consultationDetails) {
          ...
          if (consultationDetails.assignedDoctorId && consultationDetails.assignedDoctorId !== parsedDoctor.id) {
            ...
            // Set referral letters if they exist
            if (consultationDetails.referralLetters && consultationDetails.referralLetters.length > 0) {
              setReferralLetters(consultationDetails.referralLetters)
            }
  ...
  const handleSendReferral = async () => {
    ...
    try {
      ...
      // Get patient details
      const patientDetails = consultation.details || {}
      const patientName = consultation.patientName
      const patientDOB = patientDetails.dob || "Not provided"
      const patientGender = patientDetails.gender || "Not specified"
      const patientMedicare = patientDetails.medicareNumber || "Not provided"
      const patientAddress = `${patientDetails.address || ""}, ${patientDetails.suburb || ""}, ${patientDetails.state || ""} ${patientDetails.postcode || ""}`
      const patientPhone = consultation.phone || "Not provided"

      // Get doctor details
      const doctorDetails = getDoctorById(doctor.id) || doctor
      const doctorName = `Dr. ${doctorDetails.firstName} ${doctorDetails.lastName}`
      const doctorProviderNumber = doctorDetails.providerNumber || "Not provided"
      const doctorRegistrationNumber = doctorDetails.registrationNumber || "Not provided"
      const doctorSignature = doctorDetails.signature?.content || ""

      // Create referral letter object
      const referralLetter: Omit<ReferralLetter, "pdfContent"> = {
        id: `ref_${Date.now()}`,
        referenceNumber,
        date: new Date().toISOString().split("T")[0],
        referredTo: referralDetails.referredTo,
        ...
        patientAddress,
        patientPhone,
        medications: referralDetails.medications,
        allergies: referralDetails.allergies,
        medicalHistory: referralDetails.medicalHistory,
        createdAt: new Date().toISOString(),
      }

      // Generate PDF content
      const pdfContent = await generateReferralPDF(referralLetter)

      // Add PDF content to referral letter
      const completeReferralLetter = {
        ...referralLetter,
        pdfContent,
      }

      // Add to existing referral letters
      const updatedReferralLetters = [...referralLetters, completeReferralLetter]

      // Save to database
      const updatedConsultation = updateConsultRequest(consultation.id, {
        referralLetters: updatedReferralLetters,
      })

      if (updatedConsultation) {
        setReferralLetters(updatedReferralLetters)
        setConsultation(updatedConsultation)

        // Add a note about the referral
        const referralNote = `
REFERRAL LETTER GENERATED
Referred to: ${referralDetails.referredTo}
Reason: ${referralDetails.reason}
...
Date: ${new Date().toLocaleDateString()}
        ...
        toast({
          ...
          description: "The referral letter has been generated and saved successfully.",
        })
      } else {
        throw new Error("Failed to save referral letter")
      }
    } catch (error) {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to generate referral letter. Please try again.",
      })
    } finally {
...
Date Issued: ${new Date().toLocaleDateString()}
  ...
  const handleDownloadReferralLetter = (referralLetter: ReferralLetter) => {
    try {
      // Create a blob from the base64 content
      const byteCharacters = atob(referralLetter.pdfContent)
      const byteArrays = []
      ...
      a.href = url
      a.download = `Referral_${referralLetter.referenceNumber}.pdf`
      ...
      toast({
        title: "Referral Letter Downloaded",
        description: `Referral letter has been downloaded.`,
      })
    } catch (error) {
      ...
      toast({
        ...
        description: "There was an error downloading the referral letter.",
      })
    }
  }
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {referralLetters.length > 0 && (
                <div className="pt-4">
                  <p className="text-sm font-medium mb-2">Referral Letters</p>
                        ...
                        <div className="flex items-center">
                          ...
                          <div className="ml-2">
                            ...
                            <p className="text-xs text-slate-500">
                              Ref: {letter.referenceNumber} | {formatDate(letter.date)}
                            </p>
                          </div>
                        </div>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleDownloadReferralLetter(letter)}
              ...
              <Dialog>
                ...
                <DialogContent className="max-w-3xl">
                  <DialogHeader>
                    ...
                    <DialogDescription>Create a referral letter for this patient.</DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
                      <Label htmlFor="referredTo">Referral To</Label>
                      <Input
                        id="referredTo"
                        placeholder="Name of clinician or facility"
                        value={referralDetails.referredTo}
                        onChange={(e) => setReferralDetails({ ...referralDetails, referredTo: e.target.value })}
                      />
                    </div>
                    <div className="grid gap-2">
                    ...
                    <div className="grid gap-2">
                      ...
                      <Textarea
                        ...
                        value={referralDetails.medications}
                        onChange={(e) => setReferralDetails({ ...referralDetails, medications: e.target.value })}
                      />
                    </div>
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        value={referralDetails.allergies}
                        onChange={(e) => setReferralDetails({ ...referralDetails, allergies: e.target.value })}
                      />
                    </div>
                    <div className="grid gap-2">
                      ...
                      <Textarea
                        ...
                        value={referralDetails.medicalHistory}
                        onChange={(e) => setReferralDetails({ ...referralDetails, medicalHistory: e.target.value })}
                      />
                    </div>
                  </div>
                  <DialogFooter>
                    <Button onClick={handleSendReferral} disabled={isGeneratingReferral}>
                      {isGeneratingReferral ? "Generating..." : "Generate Referral Letter"}
                    </Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
